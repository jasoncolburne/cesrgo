package matter

import (
	"fmt"

	"github.com/jasoncolburne/cesrgo/types"
)

const (
	Ed25519_Seed       = types.Code("A")    // Ed25519 256 bit random seed for private key
	Ed25519N           = types.Code("B")    // Ed25519 verification key non-transferable, basic derivation.
	X25519             = types.Code("C")    // X25519 public encryption key, converted from Ed25519 or Ed25519N.
	Ed25519            = types.Code("D")    // Ed25519 verification key basic derivation
	Blake3_256         = types.Code("E")    // Blake3 256 bit digest self-addressing derivation.
	Blake2b_256        = types.Code("F")    // Blake2b 256 bit digest self-addressing derivation.
	Blake2s_256        = types.Code("G")    // Blake2s 256 bit digest self-addressing derivation.
	SHA3_256           = types.Code("H")    // SHA3 256 bit digest self-addressing derivation.
	SHA2_256           = types.Code("I")    // SHA2 256 bit digest self-addressing derivation.
	ECDSA_256k1_Seed   = types.Code("J")    // ECDSA secp256k1 256 bit random Seed for private key
	Ed448_Seed         = types.Code("K")    // Ed448 448 bit random Seed for private key
	X448               = types.Code("L")    // X448 public encryption key, converted from Ed448
	Short              = types.Code("M")    // Short 2 byte b2 number
	Big                = types.Code("N")    // Big 8 byte b2 number
	X25519_Private     = types.Code("O")    // X25519 private decryption key converted from Ed25519
	X25519_Cipher_Seed = types.Code("P")    // X25519 124 char b64 Cipher of 44 char qb64 Seed
	ECDSA_256r1_Seed   = types.Code("Q")    // ECDSA secp256r1 256 bit random Seed for private key
	Salt_128           = types.Code("0A")   // 128 bit random salt or 128 bit number (see Huge)
	Ed25519_Sig        = types.Code("0B")   // Ed25519 signature.
	ECDSA_256k1_Sig    = types.Code("0C")   // ECDSA secp256k1 signature.
	Blake3_512         = types.Code("0D")   // Blake3 512 bit digest self-addressing derivation.
	Blake2b_512        = types.Code("0E")   // Blake2b 512 bit digest self-addressing derivation.
	SHA3_512           = types.Code("0F")   // SHA3 512 bit digest self-addressing derivation.
	SHA2_512           = types.Code("0G")   // SHA2 512 bit digest self-addressing derivation.
	Long               = types.Code("0H")   // Long 4 byte b2 number
	ECDSA_256r1_Sig    = types.Code("0I")   // ECDSA secp256r1 signature.
	ECDSA_256k1N       = types.Code("1AAA") // ECDSA secp256k1 verification key non-transferable, basic derivation.
	ECDSA_256k1        = types.Code("1AAB") // ECDSA secp256k1 verification or encryption key, basic derivation
	Ed448N             = types.Code("1AAC") // Ed448 non-transferable prefix public signing verification key. Basic derivation.
	Ed448              = types.Code("1AAD") // Ed448 public signing verification key. Basic derivation.
	Ed448_Sig          = types.Code("1AAE") // Ed448 signature. Self-signing derivation.
	Tern               = types.Code("1AAF") // 3 byte b2 number or 4 char B64 str.
	DateTime           = types.Code("1AAG") // Base64 custom entypes.Coded 32 char ISO-8601 DateTime
	X25519_Cipher_Salt = types.Code("1AAH") // X25519 100 char b64 Cipher of 24 char qb64 Salt
	ECDSA_256r1N       = types.Code("1AAI") // ECDSA secp256r1 verification key non-transferable, basic derivation.
	ECDSA_256r1        = types.Code("1AAJ") // ECDSA secp256r1 verification or encryption key, basic derivation
	TBD1               = types.Code("2AAA") // Testing purposes only fixed with lead size 1
	TBD2               = types.Code("3AAA") // Testing purposes only of fixed with lead size 2
	StrB64_L0          = types.Code("4A")   // String Base64 Only Lead Size 0 (4095 * 3 | 4)
	StrB64_L1          = types.Code("5A")   // String Base64 Only Lead Size 1
	StrB64_L2          = types.Code("6A")   // String Base64 Only Lead Size 2
	StrB64_Big_L0      = types.Code("7AAA") // String Base64 Only Big Lead Size 0 (16777215 * 3 | 4)
	StrB64_Big_L1      = types.Code("8AAA") // String Base64 Only Big Lead Size 1
	StrB64_Big_L2      = types.Code("9AAA") // String Base64 Only Big Lead Size 2
	Bytes_L0           = types.Code("4B")   // Byte String Leader Size 0
	Bytes_L1           = types.Code("5B")   // Byte String Leader Size 1
	Bytes_L2           = types.Code("6B")   // Byte String Leader Size 2
	Bytes_Big_L0       = types.Code("7AAB") // Byte String Big Leader Size 0
	Bytes_Big_L1       = types.Code("8AAB") // Byte String Big Leader Size 1
	Bytes_Big_L2       = types.Code("9AAB") // Byte String Big Leader Size 2
)

var SMALL_VRZ_DEX = []byte{4, 5, 6}
var LARGE_VRZ_DEX = []byte{7, 8, 9}
var SMALL_VRZ_BYTES = uint32(3)
var LARGE_VRZ_BYTES = uint32(6)

type Sizage struct {
	Hs uint32
	Ss uint32
	Ls uint32
	Fs *uint32
}

func GetSizage(code types.Code) (Sizage, error) {
	var fs uint32

	switch code {
	case Ed25519_Seed:
		fs = 44
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Ed25519N:
		fs = 44
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case X25519:
		fs = 44
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Ed25519:
		fs = 44
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Blake3_256:
		fs = 44
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Blake2b_256:
		fs = 44
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Blake2s_256:
		fs = 44
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case SHA3_256:
		fs = 44
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case SHA2_256:
		fs = 44
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case ECDSA_256k1_Seed:
		fs = 44
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Ed448_Seed:
		fs = 76
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case X448:
		fs = 76
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Short:
		fs = 4
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Big:
		fs = 12
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case X25519_Private:
		fs = 44
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case X25519_Cipher_Seed:
		fs = 124
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case ECDSA_256r1_Seed:
		fs = 44
		return Sizage{Hs: 1, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Salt_128:
		fs = 24
		return Sizage{Hs: 2, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Ed25519_Sig:
		fs = 88
		return Sizage{Hs: 2, Ss: 0, Fs: &fs, Ls: 0}, nil
	case ECDSA_256k1_Sig:
		fs = 88
		return Sizage{Hs: 2, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Blake3_512:
		fs = 88
		return Sizage{Hs: 2, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Blake2b_512:
		fs = 88
		return Sizage{Hs: 2, Ss: 0, Fs: &fs, Ls: 0}, nil
	case SHA3_512:
		fs = 88
		return Sizage{Hs: 2, Ss: 0, Fs: &fs, Ls: 0}, nil
	case SHA2_512:
		fs = 88
		return Sizage{Hs: 2, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Long:
		fs = 8
		return Sizage{Hs: 2, Ss: 0, Fs: &fs, Ls: 0}, nil
	case ECDSA_256r1_Sig:
		fs = 88
		return Sizage{Hs: 2, Ss: 0, Fs: &fs, Ls: 0}, nil
	case ECDSA_256k1N:
		fs = 48
		return Sizage{Hs: 4, Ss: 0, Fs: &fs, Ls: 0}, nil
	case ECDSA_256k1:
		fs = 48
		return Sizage{Hs: 4, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Ed448N:
		fs = 80
		return Sizage{Hs: 4, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Ed448:
		fs = 80
		return Sizage{Hs: 4, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Ed448_Sig:
		fs = 56
		return Sizage{Hs: 4, Ss: 0, Fs: &fs, Ls: 0}, nil
	case Tern:
		fs = 8
		return Sizage{Hs: 4, Ss: 0, Fs: &fs, Ls: 0}, nil
	case DateTime:
		fs = 36
		return Sizage{Hs: 4, Ss: 0, Fs: &fs, Ls: 0}, nil
	case X25519_Cipher_Salt:
		fs = 100
		return Sizage{Hs: 4, Ss: 0, Fs: &fs, Ls: 0}, nil
	case ECDSA_256r1N:
		fs = 48
		return Sizage{Hs: 4, Ss: 0, Fs: &fs, Ls: 0}, nil
	case ECDSA_256r1:
		fs = 48
		return Sizage{Hs: 4, Ss: 0, Fs: &fs, Ls: 0}, nil
	case TBD1:
		fs = 8
		return Sizage{Hs: 4, Ss: 0, Fs: &fs, Ls: 1}, nil
	case TBD2:
		fs = 8
		return Sizage{Hs: 4, Ss: 0, Fs: &fs, Ls: 2}, nil
	case StrB64_L0:
		return Sizage{Hs: 2, Ss: 2, Ls: 0}, nil
	case StrB64_L1:
		return Sizage{Hs: 2, Ss: 2, Ls: 1}, nil
	case StrB64_L2:
		return Sizage{Hs: 2, Ss: 2, Ls: 2}, nil
	case StrB64_Big_L0:
		return Sizage{Hs: 4, Ss: 4, Ls: 0}, nil
	case StrB64_Big_L1:
		return Sizage{Hs: 4, Ss: 4, Ls: 1}, nil
	case StrB64_Big_L2:
		return Sizage{Hs: 4, Ss: 4, Ls: 2}, nil
	case Bytes_L0:
		return Sizage{Hs: 2, Ss: 2, Ls: 0}, nil
	case Bytes_L1:
		return Sizage{Hs: 2, Ss: 2, Ls: 1}, nil
	case Bytes_L2:
		return Sizage{Hs: 2, Ss: 2, Ls: 2}, nil
	case Bytes_Big_L0:
		return Sizage{Hs: 4, Ss: 4, Ls: 0}, nil
	case Bytes_Big_L1:
		return Sizage{Hs: 4, Ss: 4, Ls: 1}, nil
	case Bytes_Big_L2:
		return Sizage{Hs: 4, Ss: 4, Ls: 2}, nil
	default:
		return Sizage{}, fmt.Errorf("unknown code: %s", code)
	}
}

func Hardage(c string) (uint32, error) {
	if c >= "A" && c <= "Z" || c >= "a" && c <= "z" {
		return 1, nil
	}
	if c == "0" || c == "4" || c == "5" || c == "6" {
		return 2, nil
	}
	if c == "1" || c == "2" || c == "3" || c == "7" || c == "8" || c == "9" {
		return 4, nil
	}

	if c == "-" {
		return 0, fmt.Errorf("count code start")
	}
	if c == "_" {
		return 0, fmt.Errorf("op code start")
	}

	return 0, fmt.Errorf("unknown hardage: %s", c)
}

func Bardage(b byte) (uint32, error) {
	if b >= 0x00 && b <= 0x33 {
		return 1, nil
	}
	if b == 0x34 || b >= 0x38 && b <= 0x3a {
		return 2, nil
	}
	if b >= 0x35 && b <= 0x37 || b >= 0x3b && b <= 0x3d {
		return 4, nil
	}

	if b == 0x3e {
		return 0, fmt.Errorf("count code start")
	}
	if b == 0x3f {
		return 0, fmt.Errorf("op code start")
	}

	return 0, fmt.Errorf("unknown bardage: %x", b)
}

func RawSize(code types.Code) (uint32, error) {
	szg, err := GetSizage(code)
	if err != nil {
		return 0, err
	}
	if szg.Fs == nil {
		return 0, fmt.Errorf("cannot determine raw size for code=%s", code)
	}

	cs := szg.Hs + szg.Ss
	return (*szg.Fs-cs)*3/4 - szg.Ls, nil
}
